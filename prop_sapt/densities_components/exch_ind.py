...
def get_exch_ind_s2_density(mol: Dimer, monomer: str) -> np.ndarray:
    ...
    not_u_ra = mol.cpscf("A", perturbation=not_u_ra.T)
    ...
    not_u_sb = mol.cpscf("B", perturbation=not_u_sb.T)
    ...
    if monomer == "A":
        ...
        rho_MO_exch_ind_ra = 0.5 * (
            ...
            * oe.contract(
            ...
            + oe.contract("rR,Ra->ra", mol.omegaB_rr, not_u_ra)
            - oe.contract("Aa,rA->ra", mol.omegaB_aa, not_u_ra)
            + 2 * oe.contract("Qar,Qbs,sb->ra", mol.Qar, mol.Qbs, not_u_sb)
            # < V P2 R(X) R(V) >
            + oe.contract(
        ...
        ) + oe.contract("rsab,bs->ra", mol.t_rsab, not_u_sb.T)

        rho_MO_exch_ind[mol.slices["r"], mol.slices["a"]] = rho_MO_exch_ind_ra
        rho_MO_exch_ind[mol.slices["a"], mol.slices["r"]] = rho_MO_exch_ind_ra.T

        rho_MO_exch_ind[mol.slices["a"], mol.slices["a"]] = -oe.contract(
            "rA,ar->aA", mol.get_cpscf_ra(), not_u_ra.T
        )
        rho_MO_exch_ind[mol.slices["r"], mol.slices["r"]] = oe.contract(
            "ra,aR->rR", mol.get_cpscf_ra(), not_u_ra.T
        )
    ...
    if monomer == "B":
        ...
        rho_MO_exch_ind_sb = 0.5 * (
            ...
            * oe.contract(
            ...
            # < V P2 R([V, R(X)]) > + < V P2 R([X, R(V)]) >
            + oe.contract("sS,Sb->sb", mol.omegaA_ss, not_u_sb)
            - oe.contract("Bb,sB->sb", mol.omegaA_bb, not_u_sb)
            + 2 * oe.contract("Qar,Qbs,ra->sb", mol.Qar, mol.Qbs, not_u_ra)
            # < V P2 R(X) R(V) >
            + oe.contract(
        ...
        ) + oe.contract("rsab,ar->sb", mol.t_rsab, not_u_ra.T)

        rho_MO_exch_ind[mol.slices["s"], mol.slices["b"]] = rho_MO_exch_ind_sb
        rho_MO_exch_ind[mol.slices["b"], mol.slices["s"]] = rho_MO_exch_ind_sb.T

        rho_MO_exch_ind[mol.slices["b"], mol.slices["b"]] = -oe.contract(
            "sB,bs->bB", mol.get_cpscf_sb(), not_u_sb.T
        )
        rho_MO_exch_ind[mol.slices["s"], mol.slices["s"]] = oe.contract(
            "sb,bS->sS", mol.get_cpscf_sb(), not_u_sb.T
        )
